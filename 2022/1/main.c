...
Part_One(void)
{
    ...
    u64 calories = 0;
    u64 current_calories = 0;

    while ((line = Read_Line()) != NULL) {
        if (*line != '\0') {
            current_calories += strtoull(line, NULL, 10);
        } else {
...
Part_Two(void)
{
    ...
    while ((line = Read_Line()) != NULL) {
        if (*line != '\0') {
            ...
            if (calories_0 <= calories_1 && calories_0 <= calories_2 && current_calories > calories_0) {
                calories_0 = current_calories;
            } else if (calories_1 <= calories_2 && calories_1 <= calories_0 && current_calories > calories_1) {
                calories_1 = current_calories;
            } else if (calories_2 <= calories_0 && calories_2 <= calories_1 && current_calories > calories_2) {
    ...
    u64 calories = calories_0 + calories_1 + calories_2;

    printf("calories 0: %lu\n", calories_0);
    printf("calories 1: %lu\n", calories_1);
    printf("calories 2: %lu\n", calories_2);

    printf("Part_Two: %lu calories\n", calories);
}
